...
class Site {
    ...
    function get_site_owner($site_id) {
        if($this->site_id != $site_id) 
            $this->load_site($site_id);
    ...
    function get_site_cameras($site_id) {
     ...
     function update_site($id, $updateData) {
        foreach ($updateData as $key => $value) {
            $field_name = $key;
            $db_query   = "UPDATE {$this->db_table} SET $field_name='$value' WHERE id='$id'";
            $result     = $this->query($db_query, __LINE__);
    ...
    function update_camera($id, $updateData) {
        foreach ($updateData as $key => $value) {
            $field_name = $key;
            $db_query   = "UPDATE {$this->camera_table} SET $field_name='$value' WHERE id='$id'";
            $result     = $this->query($db_query, __LINE__);
        } //$updateData as $key => $value
        if ($result)
            return true;
