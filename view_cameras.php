				...
				<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					...
					<div class="jarviswidget jarviswidget-color-darken" id="wid-id-users">
						...
						<header>
							...
							<div class="btn-group">
								<a class="btn btn-primary" href="add_camera.php<?php echo $id_clause ?>">
									Add New Camera
								</a>								
							</div></div>
						...
						<div>
							...
							header("Location:view_cameras.php$id_clause");
							}
							elseif (isset($_GET['change_status'])) {
							    $change     = $_GET['change_status'];
							    $id    = $_GET['id'];
							    $update_sql = "UPDATE cameras SET active = '$change' WHERE id = '$id'";
							...
							$sql = "SELECT id, description, wan_ip, port, cam_user, cam_pwd, cam_num, PTZ, active FROM cameras WHERE ".$where." ORDER BY id";
						        ...
						        <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
									<thead>			                
										<tr>
											...
											<th data-hide="phone,tablet">Wan IP</th>
											<th data-hide="phone,tablet">Port</th>
											<th data-hide="phone,tablet">Is PTZ</th>
											<th>Action</th>
											<th>&nbsp;</th>
										</tr>
									</thead>
									...
									<?php if( $num > 0) { ?>
										<?php while( $row = mysql_fetch_object($result)) { 
										        if ($row->active == 0) {
										            $status_link = "<a href='$id_clause&change_status=1&amp;id=$row->id'>Activate</a>";
										        } //$row->activated == 0
										        else {
										            $status_link = "<a href='$id_clause&change_status=0&amp;id=$row->id'>De-activate</a>";
										        }
										...
										?>
											<tr>
												...
												<td><?php echo $row->cam_user ?></td>												
												<td><?php echo $row->cam_pwd ?></td>												
												<td><?php echo $row->wan_ip ?></td>												
												<td><?php echo $row->port ?></td>												
												<td><?php echo ($row->PTZ)? "YES":"NO" ?></td>												
												<td><a href="update_camera.php<?php echo $id_clause ?>&id=<?php echo $row->id; ?>">Update Record</a> </td>
