					...
					<div class="jarviswidget jarviswidget-color-darken" id="wid-id-users">
						...
						<div>
							...
							if (isset($_POST['delete'])) {
							    ...
							    $msg = "<p>Deleted " . $n . "user(s)</p>";
							} //isset($_POST['delete'])
							elseif (isset($_GET['change_status'])) {
							    $change     = $_GET['change_status'];
							    $user_id    = $_GET['id'];
							    $update_sql = "UPDATE users SET active = '$change' WHERE id = '$user_id'";
						        ...
						        <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
									<thead>			                
										<tr>
											...
											<th data-hide="email"><i class="fa fa-fw fa-envelope text-muted hidden-md hidden-sm hidden-xs"></i> Email</th>
											<th data-hide="phone,tablet"><i class="fa fa-fw fa-map-marker txt-color-blue hidden-md hidden-sm hidden-xs"></i>Sites</th>
									...
									<?php if( $num > 0) { ?>
										<?php while( $row = mysql_fetch_object($result)) { 
												if ($row->num_cameras > 0) {
										            $cameras_link = "<a href='view_cameras.php?user_id=$row->id'>($row->num_cameras) Camera(s) </a>";
										        } //$row->num_cameras > 0
										        else {
										            $cameras_link = "<a href='add_camera.php?user_id=$row->id'>Add Camera </a>";
										        }
										        if ($row->active == 0) {
										        ...
										        if ($row->num_sites > 0) {
										            ...
										            $sites_link = "<a href='add_site.php?user_id=$row->id'>Add Site </a>";
										        }
										?>
											<tr>
												...
												<td><a href="update_user.php?id=<?php echo $row->id; ?>">Update Record</a> </td>
